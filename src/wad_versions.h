...
#  define IS_WAD17_OR_HIGHER(ctx)  0
#else
#  error "You need to build Donut Spider with at least one WAD version enabled!"
#endif

static inline bool WAD_isVersionSupported(uint8_t wadVersion) {
    if (wadVersion <= 14) return IS_WAD14_ENABLED != 0;
    if (wadVersion <= 16) return IS_WAD16_OR_HIGHER_ENABLED != 0;
    return IS_WAD17_OR_HIGHER_ENABLED != 0;
}
