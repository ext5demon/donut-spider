...
static void parseGEN8(BinaryReader* reader, DataWin* dw, uint32_t chunkLength) {
    Gen8* g = &dw->gen8;
    ...
    bool isCompactWad8 = 8 >= g->wadVersion && 108 > chunkLength;
    if (isCompactWad8) {
...
static void parseEXTN(BinaryReader* reader, DataWin* dw, size_t chunkEnd) {
    Extn* e = &dw->extn;
...
static void parseOBJT(BinaryReader* reader, DataWin* dw, size_t chunkEnd) {
    Objt* o = &dw->objt;
    ...
    if (DataWin_isVersionAtLeast(dw, 2, 3, 0, 0) && !DataWin_isVersionAtLeast(dw, 2022, 5, 0, 0)) {
        ...
        if (vertexCount >= 0) {
            ...
            size_t newLocation = BinaryReader_getPosition(reader) + skipCount;
            bool isOldFormat = false;
            if (newLocation < chunkEnd) {
...
DataWin* DataWin_parse(const char* filePath, DataWinParserOptions options) {
    ...
    while ((size_t) fileSize > BinaryReader_getPosition(&reader)) {
        ...
        // Bulk-read the chunk data into memory for fast parsing unless the
        // target prefers bounded-memory streaming. Some WAD17 chunks can be
        // tens of megabytes, so the temporary copy alone can exhaust PS3 RAM.
        ...
        if (options.parseGen8 && memcmp(chunkName, "GEN8", 4) == 0) {
            parseGEN8(&reader, dw, chunkLength);
...
const uint8_t* DataWin_acquireAudoData(DataWin* dw, uint32_t entryIndex, bool* outOwned) {
    if (outOwned != nullptr) *outOwned = false;
    if (dw == nullptr || entryIndex >= dw->audo.count || dw->audo.entries == nullptr) return nullptr;

    AudioEntry* entry = &dw->audo.entries[entryIndex];
    if (!entry->present || entry->dataSize == 0) return nullptr;
    if (entry->data != nullptr) return entry->data;
    if (!dw->lazyLoadAudio || dw->lazyLoadFile == nullptr) return nullptr;

    size_t end = (size_t) entry->dataOffset + (size_t) entry->dataSize;
    if (end < entry->dataOffset || end > dw->fileSize) {
        fprintf(stderr, "AUDO entry %u points outside %s\n", entryIndex,
            dw->lazyLoadFilePath != nullptr ? dw->lazyLoadFilePath : "data.win");
        return nullptr;
    }

    uint8_t* copy = (uint8_t*) safeMalloc(entry->dataSize);
    long oldPosition = ftell(dw->lazyLoadFile);
    if (fseek(dw->lazyLoadFile, (long) entry->dataOffset, SEEK_SET) != 0) {
        free(copy);
        return nullptr;
    }
    size_t read = fread(copy, 1, entry->dataSize, dw->lazyLoadFile);
    if (oldPosition >= 0) fseek(dw->lazyLoadFile, oldPosition, SEEK_SET);
    if (read != entry->dataSize) {
        free(copy);
        return nullptr;
    }

    if (outOwned != nullptr) *outOwned = true;
    return copy;
}
...
void DataWin_free(DataWin* dw) {
    ...
    // Close the shared on-demand file handle.
