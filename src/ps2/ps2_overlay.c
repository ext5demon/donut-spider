...
// Draws a simple status screen with "Donut Spider" title, optional game name, and a status message (no progress bar)
...
static void beginStatusScreen(GSGLOBAL* gs, const char* gameName) {
    ...
    overlayPrint(320.0f, 180.0f, 1, 0.8f, title, "Donut Spider");
    if (gameName) {
...
static void drawCreditsText(void) {
    ...
    snprintf(versionText, sizeof(versionText), "Donut Spider (%s) [%s]", SPIDER_DONUT_COMMIT_HASH, SPIDER_DONUT_COMMIT_DATE);
...
void PS2Overlay_statusScreenCallback(const char* chunkName, int chunkIndex, int totalChunks, DataWin* dataWin, void* userData) {
    ...
    u64 barFg = GS_SETREG_RGBAQ(0xFF, 0xCC, 0x00, 0x80, 0x00); // Donut Spider yellow
...
#else
        overlayPrint(10.0f, profilerY, 10, 0.35f, debugColor, "Donut Spider GML Profiler is disabled on this build :(");
#endif
    }
}
