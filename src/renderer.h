...
// Compact CPU-side vertex used by the subset of GameMaker's vertex API that
// WAD17 games use for perspective-correct 2D quads. Texture coordinates are
// already normalized when this reaches a backend.
typedef struct {
    float x;
    float y;
    float u;
...
typedef struct {
    ...
    // Replace the complete contents of a surface texture with tightly-packed
    // RGBA8 pixels. Used by streamed video frames so the same surface and GPU
    // allocation can be reused for the entire clip.
    bool (*surfaceUpdatePixels)(Renderer* renderer, int32_t surfaceID, const uint8_t* rgba, int32_t width, int32_t height);
    ...
    // Optional generic textured primitive path. Currently triangle strips are
    // sufficient for the WAD17 perspective-quad helpers used by Chapter 4.
    void (*drawTexturedVertices)(Renderer* renderer, uint32_t texID,
