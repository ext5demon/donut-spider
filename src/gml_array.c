...
static GMLArray** trackedArrays;

static void registerArray(GMLArray* arr) {
    arr->gcRegistryIndex = (int32_t) arrlen(trackedArrays);
    arrput(trackedArrays, arr);
}

void GMLArray_gcUnregister(GMLArray* arr) {
    if (arr == nullptr || arr->gcRegistryIndex < 0) return;
    int32_t index = arr->gcRegistryIndex;
    int32_t lastIndex = (int32_t) arrlen(trackedArrays) - 1;
    require(index <= lastIndex && trackedArrays[index] == arr);
    if (index != lastIndex) {
        GMLArray* moved = trackedArrays[lastIndex];
        trackedArrays[index] = moved;
        moved->gcRegistryIndex = index;
    }
    (void) arrpop(trackedArrays);
    arr->gcRegistryIndex = -1;
}

size_t GMLArray_gcTrackedCount(void) {
    return arrlenu(trackedArrays);
}

GMLArray* GMLArray_gcTrackedAt(size_t index) {
    require(index < arrlenu(trackedArrays));
    return trackedArrays[index];
}

void GMLArray_gcResetMarks(void) {
    repeat(arrlen(trackedArrays), i) {
        trackedArrays[i]->gcMarked = false;
        trackedArrays[i]->gcGarbage = false;
    }
}
...
void GMLArray_decRef(GMLArray* arr) {
    if (arr == nullptr) return;
    // A mark-and-sweep pass destroys garbage in two phases. Suppress recursive
    // frees while graph edges are being disconnected so cycles remain safe.
    if (arr->gcGarbage) {
        if (arr->refCount > 0) arr->refCount--;
        return;
    }
...
void GMLArray_gcClearGarbage(GMLArray* arr) {
    require(arr != nullptr && arr->gcGarbage && arr->gcRegistryIndex < 0);
    if (arr->type == GML_LEGACY_ARRAY) {
        repeat(arr->legacy.rowCount, r) {
            GMLArrayRow* row = &arr->legacy.rows[r];
        ...
        free(arr->legacy.rows);
        arr->legacy.rows = nullptr;
        arr->legacy.rowCount = 0;
        arr->legacy.rowCapacity = 0;
    } else {
        repeat(arr->modern.length, i) RValue_free(&arr->modern.data[i]);
        free(arr->modern.data);
        arr->modern.data = nullptr;
        arr->modern.length = 0;
        arr->modern.capacity = 0;
    }
}

void GMLArray_gcFreeGarbage(GMLArray* arr) {
    require(arr != nullptr && arr->gcGarbage && arr->gcRegistryIndex < 0);
    free(arr);
}
