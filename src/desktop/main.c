...
typedef struct {
    ...
    const char* sharedFolder; // optional read-only fallback for files shared between games/chapters
...
static void printUsage(const char *argv0) {
    fprintf(
        ...
        "    --shared-folder <directory>            - Fall back to a shared read-only bundle directory\n"
...
static void parseCommandLineArgs(CommandLineArgs* args, int argc, char* argv[]) {
    ...
    static struct option longOptions[] = {
        ...
        {"shared-folder", required_argument, nullptr, 1001},
...
InputRecording* globalInputRecording = nullptr;
        ...
        #define SPIDER_DONUT_HAS_ASAN 1
    #endif
#endif
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        if (!WAD_isVersionSupported(gen8->wadVersion)) {
            fprintf(stderr, "Unsupported WAD version %u in this build. Rebuild with the matching ENABLE_WAD option.\n", gen8->wadVersion);
        ...
        snprintf(windowTitle, sizeof(windowTitle), "Donut Spider - %s", gen8->displayName);
        ...
        if (args.sharedFolder != nullptr) {
            OverlayFileSystem_setFallbackBundlePath(overlayFs, args.sharedFolder);
            fprintf(stderr, "Using shared bundle fallback: %s\n", args.sharedFolder);
        }
        free(dataWinDir);
