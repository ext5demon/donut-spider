...
static inline RValue RValue_makeIndependent(RValue val) {
    ...
    requireMessageFormatted(__FILE__, __LINE__, !val.ownsReference, "Trying to make independent a RValue (type=%d) that owns a reference, but we don't handle it yet! Did you add a new refcounted value to Donut Spider without implementing RValue_makeIndependent for it?", val.type);
    return val;
}
