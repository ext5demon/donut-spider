...
void Runner_draw(Runner* runner) {
    ...
    ParticleManager_drawAutomatic(runner->particleManager, runner);
...
static void cleanupState(Runner* runner) {
    // Video owns a streamed file, an audio stream, and a reusable GPU surface.
    // Release them while all three platform backends are still alive.
    VideoPlayer_close(runner->videoPlayer, runner);

    ParticleManager_free(runner->particleManager);
    runner->particleManager = nullptr;

    VertexManager_free(runner->vertexManager);
    runner->vertexManager = nullptr;
...
Runner* Runner_create(DataWin* dataWin, VMContext* vm, Renderer* renderer, FileSystem* fileSystem, AudioSystem* audioSystem) {
    ...
    runner->videoPlayer = VideoPlayer_create();
...
// Reference counting handles the hot acyclic path every frame. A periodic
// mark-and-sweep pass handles array/struct cycles without adding work to every
// RValue assignment. Sixty frames bounds cyclic garbage retention to roughly
// one second in a typical room.
static void Runner_collectCyclicGarbage(Runner* runner, bool force) {
    if (!force && (runner->frameCount % 60) != 0) return;
    GMLGCStats stats = GMLGC_collect(runner);
#ifdef ENABLE_GC_LOGS
    fprintf(stderr,
            "GC: arrays=%zu/%zu structs=%zu/%zu collected\n",
            stats.arraysCollected, stats.arraysBefore,
            stats.structsCollected, stats.structsBefore);
#else
    (void) stats;
#endif
}
...
void Runner_handlePendingRoomChange(Runner* runner) {
    ...
    if (runner->pendingRoom >= 0) {
        ...
        Runner_collectCyclicGarbage(runner, true);
    }
}
...
void Runner_step(Runner* runner) {
    ...
    VideoPlayer_update(runner->videoPlayer, runner);
    const char* videoEventType;
    while ((videoEventType = VideoPlayer_pollAsyncEvent(runner->videoPlayer)) != nullptr) {
        ...
        shput(*mapPtr, safeStrdup("type"), RValue_makeOwnedString(safeStrdup(videoEventType)));
    ...
    // Automatic particle updates continue even when automatic drawing is
    // disabled. Games commonly draw those systems into a surface themselves.
    ParticleManager_updateAutomatic(runner->particleManager);
    ...
    Runner_cleanupDestroyedInstances(runner);
    Runner_sweepDeadStructs(runner);
    Runner_collectCyclicGarbage(runner, false);
...
void Runner_free(Runner* runner) {
    ...
    VideoPlayer_free(runner->videoPlayer, runner);
    runner->videoPlayer = nullptr;
