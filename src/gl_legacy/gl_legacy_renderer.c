...
static void glInit(Renderer* renderer, DataWin* dataWin) {
    ...
    gl->textureLastUsed = (uint64_t *)safeCalloc(gl->textureCount, sizeof(uint64_t));
    gl->textureBytes = (size_t *)safeCalloc(gl->textureCount, sizeof(size_t));
#ifdef PLATFORM_PS3
    gl->textureCacheBudget = 96u * 1024u * 1024u;
#else
    gl->textureCacheBudget = (size_t) -1;
#endif
...
static void glDestroy(Renderer* renderer) {
    ...
    free(gl->textureLastUsed);
    free(gl->textureBytes);
    free(gl);
}
...
static void glBeginGUI(Renderer* renderer, int32_t guiW, int32_t guiH, int32_t portX, int32_t portY, int32_t portW, int32_t portH, int32_t targetSurfaceId) {
    ...
    if (targetSurfaceId == RENDER_TARGET_HOST_FRAMEBUFFER) {
        ...
        gl->activeSurfaceId = targetSurfaceId;
        glApplyViewport(gl, portX, portY, portW, portH);
    }
...
static void glLegacyEvictTexturePage(GLLegacyRenderer* gl, uint32_t pageId) {
    if (pageId >= gl->originalTexturePageCount || !gl->textureLoaded[pageId]) return;
    glBindTexture(GL_TEXTURE_2D, 0);
    ...
    gl->textureLastUsed[pageId] = 0;
    if (gl->textureBytesResident >= gl->textureBytes[pageId]) {
        gl->textureBytesResident -= gl->textureBytes[pageId];
    } else {
        gl->textureBytesResident = 0;
    }
    gl->textureBytes[pageId] = 0;
}

static void glLegacyMakeTextureCacheRoom(GLLegacyRenderer* gl, uint32_t incomingPageId, size_t incomingBytes) {
#ifndef PLATFORM_PS3
    if (!gl->base.dataWin->lazyLoadTextures) return;
#endif
    while (gl->textureBytesResident + incomingBytes > gl->textureCacheBudget) {
        uint32_t candidate = UINT32_MAX;
        uint64_t oldestUse = UINT64_MAX;
        for (uint32_t i = 0; i < gl->originalTexturePageCount; i++) {
            if (i == incomingPageId || !gl->textureLoaded[i]) continue;
            if (gl->textureLastUsed[i] < oldestUse) {
                oldestUse = gl->textureLastUsed[i];
                candidate = i;
            }
        }
        if (candidate == UINT32_MAX) break;
        glLegacyEvictTexturePage(gl, candidate);
    }
}
...
bool GLLegacyRenderer_ensureTextureLoaded(GLLegacyRenderer* gl, uint32_t pageId) {
    if (gl->textureLoaded[pageId]) {
        gl->textureLastUsed[pageId] = ++gl->textureUseCounter;
    ...
    size_t pageBytes = (size_t) w * (size_t) h;
    glLegacyMakeTextureCacheRoom(gl, pageId, pageBytes);
    ...
    size_t pageBytes = (size_t) w * (size_t) h * 4u;
    glLegacyMakeTextureCacheRoom(gl, pageId, pageBytes);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
#endif
    gl->textureBytes[pageId] = pageBytes;
    gl->textureBytesResident += pageBytes;
    gl->textureLastUsed[pageId] = ++gl->textureUseCounter;
...
static uint32_t findOrAllocTexturePageSlot(GLLegacyRenderer* gl) {
    ...
    gl->textureLastUsed = (uint64_t *)safeRealloc(gl->textureLastUsed, gl->textureCount * sizeof(uint64_t));
    gl->textureBytes = (size_t *)safeRealloc(gl->textureBytes, gl->textureCount * sizeof(size_t));
    gl->glTextures[newPageId] = 0;
    gl->textureWidths[newPageId] = 0;
    gl->textureHeights[newPageId] = 0;
    gl->textureLoaded[newPageId] = false;
    gl->textureLastUsed[newPageId] = 0;
    gl->textureBytes[newPageId] = 0;
    return newPageId;
}
...
static void measureAlpha(const uint8_t* pixels, size_t pixelCount,
                         uint8_t* outMin, uint8_t* outMax,
                         uint32_t* outZero, uint32_t* outPartial, uint32_t* outOpaque) {
    *outMin = 255;
    *outMax = 0;
    *outZero = 0;
    *outPartial = 0;
    *outOpaque = 0;
    for (size_t i = 0; i < pixelCount; i++) {
        uint8_t alpha = pixels[i * 4u + 3u];
        if (alpha < *outMin) *outMin = alpha;
        if (alpha > *outMax) *outMax = alpha;
        if (alpha == 0) (*outZero)++;
        else if (alpha == 255) (*outOpaque)++;
        else (*outPartial)++;
    }
}
#endif

// GameMaker's removeback flag ignores source alpha, chooses the bottom-left
// pixel as the background colour, and makes exact RGB matches transparent.
static void applySurfaceSpriteRemoveBack(uint8_t* pixels, int32_t w, int32_t h,
                                         uint8_t* outR, uint8_t* outG, uint8_t* outB) {
#ifdef PLATFORM_PS3
    // The PS3 readback shim returns rows top-down.
    const size_t backgroundIndex = ((size_t)h - 1u) * (size_t)w * 4u;
#else
    // Standard glReadPixels returns the bottom row first.
    const size_t backgroundIndex = 0;
#endif
    const uint8_t backgroundR = pixels[backgroundIndex + 0u];
    const uint8_t backgroundG = pixels[backgroundIndex + 1u];
    const uint8_t backgroundB = pixels[backgroundIndex + 2u];
    *outR = backgroundR;
    *outG = backgroundG;
    *outB = backgroundB;
    ...
    for (size_t i = 0; i < pixelCount; i++) {
        uint8_t* pixel = pixels + i * 4u;
        pixel[3] = (pixel[0] == backgroundR && pixel[1] == backgroundG && pixel[2] == backgroundB) ? 0 : 255;
    }
}
...
static int32_t glCreateSpriteFromSurface(Renderer* renderer, int32_t surfaceID, int32_t x, int32_t y, int32_t w, int32_t h, bool removeback, bool smooth, int32_t xorig, int32_t yorig) {
...
#ifdef DONUT_SPIDER_DEV_BUILD
    gl->captureSerial++;
    gl->lastCaptureSurfaceId = surfaceID;
    gl->lastCaptureX = x;
    gl->lastCaptureY = y;
    gl->lastCaptureWidth = w;
    gl->lastCaptureHeight = h;
    gl->lastCaptureRemoveBack = removeback;
    gl->lastCaptureSmooth = smooth;
    gl->lastCaptureBlendEnabled = glIsEnabled(GL_BLEND);
    gl->lastCaptureAlphaTestEnabled = glIsEnabled(GL_ALPHA_TEST);
    gl->lastCaptureWriteAlpha = gl->colorWriteA;
    gl->lastCaptureBlendMode = gl->currentBlendMode;
    gl->lastCaptureSFactor = gl->currentSFactor;
    gl->lastCaptureDFactor = gl->currentDFactor;
    gl->lastCaptureSFactorAlpha = gl->currentSFactorAlpha;
    gl->lastCaptureDFactorAlpha = gl->currentDFactorAlpha;
    const size_t capturePixels = (size_t)w * (size_t)h;
    measureAlpha(pixels, capturePixels,
                 &gl->lastCaptureAlphaMin, &gl->lastCaptureAlphaMax,
    ...
    if (removeback) {
        applySurfaceSpriteRemoveBack(pixels, w, h,
                                     &backgroundR,
                                     &backgroundG,
                                     &backgroundB);
    }
...
#ifdef DONUT_SPIDER_DEV_BUILD
    gl->lastCaptureBackgroundR = backgroundR;
    gl->lastCaptureBackgroundG = backgroundG;
    gl->lastCaptureBackgroundB = backgroundB;
    measureAlpha(pixels, capturePixels,
                 &gl->lastCaptureOutputAlphaMin, &gl->lastCaptureOutputAlphaMax,
                 &gl->lastCaptureOutputAlphaZero, &gl->lastCaptureOutputAlphaPartial,
                 &gl->lastCaptureOutputAlphaOpaque);
    fprintf(stderr,
        "GLDBG: capture #%u surface=%d crop=(%d,%d %dx%d) source=%dx%d removeback=%d smooth=%d bg=#%02X%02X%02X state(blend=%d alphaTest=%d writeA=%d mode=%d factors=%d,%d,%d,%d) inputA=[%u..%u] (%u/%u/%u) outputA=[%u..%u] (%u/%u/%u)\n",
        gl->captureSerial, surfaceID, x, y, w, h,
        gl->surfaceWidth[surfaceID], gl->surfaceHeight[surfaceID],
        removeback ? 1 : 0, smooth ? 1 : 0,
        (unsigned)gl->lastCaptureBackgroundR, (unsigned)gl->lastCaptureBackgroundG, (unsigned)gl->lastCaptureBackgroundB,
        gl->lastCaptureBlendEnabled ? 1 : 0, gl->lastCaptureAlphaTestEnabled ? 1 : 0,
        gl->lastCaptureWriteAlpha ? 1 : 0, gl->lastCaptureBlendMode,
...
static bool glLegacySurfaceUpdatePixels(Renderer* renderer, int32_t surfaceId, const uint8_t* rgba, int32_t width, int32_t height) {
    GLLegacyRenderer* gl = (GLLegacyRenderer*) renderer;
    if (rgba == nullptr || width <= 0 || height <= 0) return false;
    if (surfaceId < 0 || (uint32_t) surfaceId >= gl->surfaceCount || gl->surfaceTexture[surfaceId] == 0) return false;
    if (gl->surfaceWidth[surfaceId] != width || gl->surfaceHeight[surfaceId] != height) return false;
...
static bool glLegacyResolveTextureHandle(GLLegacyRenderer* gl, uint32_t texHandle, TexturePageItem** outTpag, GLuint* outTexId, int32_t* outW, int32_t* outH) {
    ...
    if (texHandle & GL_SURFACE_TEXTURE_FLAG) {
        ...
        if (outTexId) *outTexId = gl->surfaceTexture[sid];
    ...
    if (!GLLegacyRenderer_ensureTextureLoaded(gl, (uint32_t) pageId)) return false;
    *outTpag = tpag;
    if (outTexId) *outTexId = gl->glTextures[pageId];
...
static float glTextureGetTexelWidth(Renderer* renderer, uint32_t texHandle) {
    ...
    int32_t w = 0, h = 0;
    if (!glLegacyResolveTextureHandle(gl, texHandle, &tpag, nullptr, &w, &h) || 0 >= w) return 1.0f;
    return 1.0f / (float) w;
}
...
static float glTextureGetTexelHeight(Renderer* renderer, uint32_t texHandle) {
    ...
    TexturePageItem* tpag;
    int32_t w = 0, h = 0;
    if (!glLegacyResolveTextureHandle(gl, texHandle, &tpag, nullptr, &w, &h) || 0 >= h) return 1.0f;
    return 1.0f / (float) h;
}
...
static bool glTextureGetUVs(Renderer* renderer, uint32_t texHandle, float* outUVs) {
    ...
    TexturePageItem* tpag;
    int32_t w = 0, h = 0;
    if (!glLegacyResolveTextureHandle(gl, texHandle, &tpag, nullptr, &w, &h) || 0 >= w || 0 >= h) return false;
...
static void glLegacyDrawTexturedVertices(Renderer* renderer, uint32_t texHandle,
                                         const RendererVertex2D* vertices,
                                         int32_t vertexCount, int32_t primitiveKind) {
    if (vertices == nullptr || vertexCount < 3 || primitiveKind != 5) return;
    GLLegacyRenderer* gl = (GLLegacyRenderer*) renderer;
    TexturePageItem* tpag = nullptr;
    GLuint texId = 0;
    int32_t w = 0, h = 0;
    if (!glLegacyResolveTextureHandle(gl, texHandle, &tpag, &texId, &w, &h)) return;

    glBindTexture(GL_TEXTURE_2D, texId);
    int32_t tpagIndex = (texHandle & GL_SURFACE_TEXTURE_FLAG) ? -1 : (int32_t) texHandle - 1;
    if (tpagIndex >= 0) PS3_PALETTED_BEGIN(tpagIndex);
    glBegin(GL_TRIANGLE_STRIP);
    for (int32_t i = 0; i < vertexCount; ++i) {
        const RendererVertex2D* vertex = &vertices[i];
        glColor4f((float) BGR_R(vertex->color) / 255.0f,
                  (float) BGR_G(vertex->color) / 255.0f,
                  (float) BGR_B(vertex->color) / 255.0f,
                  vertex->alpha);
        // q preserves the perspective correction encoded by the WAD17 helper's
        // vertex_normal(u*q, v*q, q) calls.
        glTexCoord4f(vertex->u * vertex->q, vertex->v * vertex->q, 0.0f, vertex->q);
        glVertex2f(vertex->x, vertex->y);
    }
    glEnd();
    if (tpagIndex >= 0) PS3_PALETTED_END();
}
...
Renderer* GLLegacyRenderer_create(void) {
    ...
    glVtable.surfaceUpdatePixels = glLegacySurfaceUpdatePixels;
    ...
    glVtable.drawTexturedVertices = glLegacyDrawTexturedVertices;
