...
RValue VMBuiltins_getVariable(VMContext* ctx, Instance* inst, int16_t builtinVarId, const char* name, int32_t arrayIndex) {
    ...
    // In the past Donut Spider used cascading ifs for this, which in my opinion looked nicer AND GCC was converting the ifs into a jump table, so it was all well...
...
static RValue builtin_audio_group_set_gain(VMContext* ctx, RValue* args, int32_t argCount) {
    AudioSystem* audio = getAudioSystem(ctx);
    if (audio == nullptr || argCount < 2) return RValue_makeUndefined();

    int32_t groupIndex = RValue_toInt32(args[0]);
    float gain = (float) RValue_toReal(args[1]);
    GMLReal requestedTime = argCount >= 3 ? RValue_toReal(args[2]) : 0.0;
    uint32_t timeMs = requestedTime > 0.0 ? (uint32_t) requestedTime : 0;
    AudioSystem_storeGroupGain(audio, groupIndex, gain);
    if (audio->vtable->setGroupGain != nullptr) {
...
// Particle systems. The implementation lives outside the VM so every system,
// emitter, and particle has one bounded owner and is released on restart/exit.
static ParticleManager* getParticleManager(VMContext* ctx) {
    if (ctx == nullptr || ctx->runner == nullptr) return nullptr;
    if (ctx->runner->particleManager == nullptr) {
        ctx->runner->particleManager = ParticleManager_create();
    }
    return ctx->runner->particleManager;
}

static RValue builtin_part_system_create(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    return RValue_makeReal((GMLReal) ParticleManager_createSystem(getParticleManager(ctx)));
}

static RValue builtin_part_system_destroy(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 1) ParticleManager_destroySystem(getParticleManager(ctx), RValue_toInt32(args[0]));
    return RValue_makeUndefined();
}

static RValue builtin_part_system_automatic_draw(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 2) ParticleManager_setAutomaticDraw(getParticleManager(ctx), RValue_toInt32(args[0]), RValue_toBool(args[1]));
    return RValue_makeUndefined();
}

static RValue builtin_part_system_depth(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 2) ParticleManager_setSystemDepth(getParticleManager(ctx), RValue_toInt32(args[0]), RValue_toInt32(args[1]));
    return RValue_makeUndefined();
}

static RValue builtin_part_system_update(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 1) ParticleManager_updateSystem(getParticleManager(ctx), RValue_toInt32(args[0]));
    return RValue_makeUndefined();
}

static RValue builtin_part_system_drawit(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 1) ParticleManager_drawSystem(getParticleManager(ctx), ctx->runner, RValue_toInt32(args[0]));
    return RValue_makeUndefined();
}

static RValue builtin_part_type_create(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    return RValue_makeReal((GMLReal) ParticleManager_createType(getParticleManager(ctx)));
}

static RValue builtin_part_type_destroy(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 1) ParticleManager_destroyType(getParticleManager(ctx), RValue_toInt32(args[0]));
    return RValue_makeUndefined();
}

static RValue builtin_part_type_sprite(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 5) ParticleManager_setTypeSprite(getParticleManager(ctx), RValue_toInt32(args[0]),
        RValue_toInt32(args[1]), RValue_toBool(args[2]), RValue_toBool(args[3]), RValue_toBool(args[4]));
    return RValue_makeUndefined();
}

static RValue builtin_part_type_speed(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 5) ParticleManager_setTypeSpeed(getParticleManager(ctx), RValue_toInt32(args[0]),
        (float) RValue_toReal(args[1]), (float) RValue_toReal(args[2]),
        (float) RValue_toReal(args[3]), (float) RValue_toReal(args[4]));
    return RValue_makeUndefined();
}

static RValue builtin_part_type_direction(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 5) ParticleManager_setTypeDirection(getParticleManager(ctx), RValue_toInt32(args[0]),
        (float) RValue_toReal(args[1]), (float) RValue_toReal(args[2]),
        (float) RValue_toReal(args[3]), (float) RValue_toReal(args[4]));
    return RValue_makeUndefined();
}

static RValue builtin_part_type_life(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 3) ParticleManager_setTypeLife(getParticleManager(ctx), RValue_toInt32(args[0]),
        RValue_toInt32(args[1]), RValue_toInt32(args[2]));
    return RValue_makeUndefined();
}

static RValue builtin_part_type_alpha3(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 4) ParticleManager_setTypeAlpha3(getParticleManager(ctx), RValue_toInt32(args[0]),
        (float) RValue_toReal(args[1]), (float) RValue_toReal(args[2]), (float) RValue_toReal(args[3]));
    return RValue_makeUndefined();
}

static RValue builtin_part_type_size(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 5) ParticleManager_setTypeSize(getParticleManager(ctx), RValue_toInt32(args[0]),
        (float) RValue_toReal(args[1]), (float) RValue_toReal(args[2]),
        (float) RValue_toReal(args[3]), (float) RValue_toReal(args[4]));
    return RValue_makeUndefined();
}

static RValue builtin_part_type_blend(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 2) ParticleManager_setTypeBlend(getParticleManager(ctx), RValue_toInt32(args[0]), RValue_toBool(args[1]));
    return RValue_makeUndefined();
}

static RValue builtin_part_type_gravity(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 3) ParticleManager_setTypeGravity(getParticleManager(ctx), RValue_toInt32(args[0]),
        (float) RValue_toReal(args[1]), (float) RValue_toReal(args[2]));
    return RValue_makeUndefined();
}

static RValue builtin_part_emitter_create(VMContext* ctx, RValue* args, int32_t argCount) {
    int32_t id = argCount >= 1 ? ParticleManager_createEmitter(getParticleManager(ctx), RValue_toInt32(args[0])) : -1;
    return RValue_makeReal((GMLReal) id);
}

static RValue builtin_part_emitter_destroy_all(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 1) ParticleManager_destroyAllEmitters(getParticleManager(ctx), RValue_toInt32(args[0]));
    return RValue_makeUndefined();
}

static RValue builtin_part_emitter_region(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 8) ParticleManager_setEmitterRegion(getParticleManager(ctx),
...
static RValue builtin_part_emitter_stream(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 4) ParticleManager_streamEmitter(getParticleManager(ctx),
        RValue_toInt32(args[0]), RValue_toInt32(args[1]), RValue_toInt32(args[2]), RValue_toInt32(args[3]));
    return RValue_makeUndefined();
}

static RValue builtin_part_emitter_burst(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 4) ParticleManager_burstEmitter(getParticleManager(ctx),
        RValue_toInt32(args[0]), RValue_toInt32(args[1]), RValue_toInt32(args[2]), RValue_toInt32(args[3]));
    return RValue_makeUndefined();
}

// Texture-group prefetching is only an optimization. Texture pages are loaded
// on demand by the renderer, so returning an empty array is correct for the
// current parser (which intentionally skips TGIN metadata) and avoids retaining
// a second texture-page index for memory-constrained consoles.
static RValue builtin_texturegroup_get_textures(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    return RValue_makeArray(GMLArray_create(ctx->dataWin->gen8.wadVersion, 0));
}

// Borderless desktop presentation has no equivalent on a console. Treat the
// request as a successful no-op instead of logging it as an unknown API.
static RValue builtin_window_enable_borderless_fullscreen(MAYBE_UNUSED VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    return RValue_makeUndefined();
}

// WAD17 vertex buffers. These are deliberately CPU-backed: Chapter 4 creates
// four vertices and immediately submits/deletes them, so bounded staging memory
// avoids retaining driver buffers on the PS3's small graphics heap.
static VertexManager* getVertexManager(VMContext* ctx) {
    if (ctx == nullptr || ctx->runner == nullptr) return nullptr;
    if (ctx->runner->vertexManager == nullptr) ctx->runner->vertexManager = VertexManager_create();
    return ctx->runner->vertexManager;
}

static RValue builtin_vertex_format_begin(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    VertexManager_formatBegin(getVertexManager(ctx));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_format_add_position(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    VertexManager_formatAddPosition(getVertexManager(ctx));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_format_add_position_3d(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    VertexManager_formatAddPosition3D(getVertexManager(ctx));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_format_add_colour(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    VertexManager_formatAddColor(getVertexManager(ctx));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_format_add_normal(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    VertexManager_formatAddNormal(getVertexManager(ctx));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_format_add_texcoord(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    VertexManager_formatAddTexcoord(getVertexManager(ctx));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_format_end(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    return RValue_makeReal((GMLReal) VertexManager_formatEnd(getVertexManager(ctx)));
}
static RValue builtin_vertex_create_buffer(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    return RValue_makeReal((GMLReal) VertexManager_createBuffer(getVertexManager(ctx)));
}
static RValue builtin_vertex_delete_buffer(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 1) VertexManager_deleteBuffer(getVertexManager(ctx), RValue_toInt32(args[0]));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_begin(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 2) VertexManager_begin(getVertexManager(ctx), RValue_toInt32(args[0]), RValue_toInt32(args[1]));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_colour(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 3) VertexManager_color(getVertexManager(ctx), RValue_toInt32(args[0]),
        (uint32_t) RValue_toInt32(args[1]), (float) RValue_toReal(args[2]));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_position(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 3) VertexManager_position(getVertexManager(ctx), RValue_toInt32(args[0]),
        (float) RValue_toReal(args[1]), (float) RValue_toReal(args[2]));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_position_3d(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 4) VertexManager_position3D(getVertexManager(ctx), RValue_toInt32(args[0]),
        (float) RValue_toReal(args[1]), (float) RValue_toReal(args[2]), (float) RValue_toReal(args[3]));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_normal(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 4) VertexManager_normal(getVertexManager(ctx), RValue_toInt32(args[0]),
        (float) RValue_toReal(args[1]), (float) RValue_toReal(args[2]), (float) RValue_toReal(args[3]));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_texcoord(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 3) VertexManager_texcoord(getVertexManager(ctx), RValue_toInt32(args[0]),
        (float) RValue_toReal(args[1]), (float) RValue_toReal(args[2]));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_end(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 1) VertexManager_end(getVertexManager(ctx), RValue_toInt32(args[0]));
    return RValue_makeUndefined();
}
static RValue builtin_vertex_submit(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 3) VertexManager_submit(getVertexManager(ctx), ctx->runner->renderer,
        RValue_toInt32(args[0]), RValue_toInt32(args[1]), (uint32_t) RValue_toInt32(args[2]));
    return RValue_makeUndefined();
}
...
static RValue builtin_video_open(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount < 1 || ctx->runner == nullptr) return RValue_makeUndefined();
    char* path = RValue_toString(args[0]);
    VideoPlayer_open(ctx->runner->videoPlayer, ctx->runner, path);
    free(path);
    return RValue_makeUndefined();
}

// Compare two array entries using GameMaker's built-in scalar ordering.  This
// is used when array_sort's second argument is a boolean rather than a method.
static int32_t arraySortCompareScalars(RValue left, RValue right) {
    if (left.type == RVALUE_STRING && right.type == RVALUE_STRING) {
        const char* leftString = left.string != nullptr ? left.string : "";
        const char* rightString = right.string != nullptr ? right.string : "";
        int32_t result = strcmp(leftString, rightString);
        return (result > 0) - (result < 0);
    }

    GMLReal leftNumber = RValue_toReal(left);
    GMLReal rightNumber = RValue_toReal(right);
...
#if IS_WAD17_OR_HIGHER_ENABLED
// Invoke an array_sort comparator method without transferring ownership of the
// two array slots. VM_callCodeIndex makes independent argument copies before it
// executes the comparator.
static int32_t arraySortCompareMethod(VMContext* ctx, GMLMethod* method, RValue left, RValue right) {
    if (method == nullptr) return 0;

    Instance* savedSelf = ctx->currentInstance;
    if (method->boundInstanceId > 0) {
        Instance* boundSelf = VM_findInstanceByTarget(ctx, method->boundInstanceId);
        if (boundSelf != nullptr) ctx->currentInstance = boundSelf;
    }

    RValue compareArgs[2] = {left, right};
    RValue result = RValue_makeUndefined();
    if (method->codeIndex >= 0 && (uint32_t) method->codeIndex < ctx->dataWin->code.count) {
        result = VM_callCodeIndex(ctx, method->codeIndex, compareArgs, 2);
    } else if (method->builtin != nullptr) {
        BuiltinFunc builtin = (BuiltinFunc) method->builtin;
        result = builtin(ctx, compareArgs, 2);
    }

    ctx->currentInstance = savedSelf;
    GMLReal numericResult = RValue_toReal(result);
    RValue_free(&result);
    return (numericResult > 0) - (numericResult < 0);
}
#endif

// array_sort(array, ascending_or_comparator) - stable in-place sort.  A small
// insertion sort is intentional here: GameMaker arrays used for draw ordering
// are normally short, and a context-free qsort callback cannot safely invoke a
// re-entrant GML comparator method.
static RValue builtin_array_sort(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount < 2 || args[0].type != RVALUE_ARRAY || args[0].array == nullptr) {
        return RValue_makeUndefined();
    }

    GMLArray* array = args[0].array;
    int32_t length = GMLArray_length1D(array);
    bool ascending = true;
#if IS_WAD17_OR_HIGHER_ENABLED
    GMLMethod* comparator = args[1].type == RVALUE_METHOD ? args[1].method : nullptr;
#else
    void* comparator = nullptr;
#endif
    if (comparator == nullptr) ascending = RValue_toBool(args[1]);

    for (int32_t i = 1; i < length; i++) {
        int32_t j = i;
        while (j > 0) {
            RValue* left = GMLArray_slot(array, j - 1);
            RValue* right = GMLArray_slot(array, j);
            if (left == nullptr || right == nullptr) break;

            int32_t comparison;
#if IS_WAD17_OR_HIGHER_ENABLED
            if (comparator != nullptr) {
                comparison = arraySortCompareMethod(ctx, comparator, *left, *right);
            } else
#endif
            {
                comparison = arraySortCompareScalars(*left, *right);
                if (!ascending) comparison = -comparison;
            }

            if (comparison <= 0) break;
            RValue temporary = *left;
            *left = *right;
            *right = temporary;
            j--;
        }
...
static RValue builtin_video_close(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    if (ctx->runner != nullptr) VideoPlayer_close(ctx->runner->videoPlayer, ctx->runner);
    return RValue_makeUndefined();
}
...
static RValue builtin_video_draw(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    ...
    if (ctx->runner != nullptr) {
        status = VideoPlayer_draw(ctx->runner->videoPlayer, ctx->runner, &surfaceId);
    }

    int32_t outputLength = status == 0 ? 2 : 1;
    GMLArray* output = GMLArray_create(ctx->dataWin->gen8.wadVersion, outputLength);
    GMLArray_set(output, 0, RValue_makeReal((GMLReal) status));
    if (status == 0) GMLArray_set(output, 1, RValue_makeReal((GMLReal) surfaceId));
    return RValue_makeArray(output);
}

static RValue builtin_video_enable_loop(VMContext* ctx, RValue* args, int32_t argCount) {
    if (ctx->runner != nullptr && argCount > 0) {
        VideoPlayer_setLooping(ctx->runner->videoPlayer, RValue_toBool(args[0]));
    }
    return RValue_makeUndefined();
}
...
static RValue builtin_video_set_volume(VMContext* ctx, RValue* args, int32_t argCount) {
    if (ctx->runner != nullptr && argCount > 0) {
        VideoPlayer_setVolume(ctx->runner->videoPlayer, ctx->runner, (float) RValue_toReal(args[0]));
    }
    return RValue_makeUndefined();
}

static RValue builtin_video_get_duration(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    return RValue_makeReal((GMLReal) VideoPlayer_getDurationMs(ctx->runner != nullptr ? ctx->runner->videoPlayer : nullptr));
}

static RValue builtin_video_get_position(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    return RValue_makeReal((GMLReal) VideoPlayer_getPositionMs(ctx->runner != nullptr ? ctx->runner->videoPlayer : nullptr));
}

static RValue builtin_video_get_status(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    return RValue_makeReal((GMLReal) VideoPlayer_getStatus(ctx->runner != nullptr ? ctx->runner->videoPlayer : nullptr));
}

static RValue builtin_video_get_format(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    return RValue_makeReal((GMLReal) VideoPlayer_getFormat(ctx->runner != nullptr ? ctx->runner->videoPlayer : nullptr));
}

static RValue builtin_video_pause(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    if (ctx->runner != nullptr) VideoPlayer_pause(ctx->runner->videoPlayer, ctx->runner);
    return RValue_makeUndefined();
}

static RValue builtin_video_resume(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    if (ctx->runner != nullptr) VideoPlayer_resume(ctx->runner->videoPlayer, ctx->runner);
    return RValue_makeUndefined();
}
...
static RValue builtin_mp_grid_path(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    if (cxs < 0 || cxs >= mp->hcells || cys < 0 || cys >= mp->vcells ||
        cxg < 0 || cxg >= mp->hcells || cyg < 0 || cyg >= mp->vcells) {
        fprintf(stderr,
            "MP grid path failed: out of bounds start=(%.1f,%.1f)->(%d,%d) goal=(%.1f,%.1f)->(%d,%d) grid=%dx%d origin=(%.1f,%.1f) cell=(%.1f,%.1f)\n",
            (double) xstart, (double) ystart, cxs, cys,
            (double) xgoal, (double) ygoal, cxg, cyg,
            mp->hcells, mp->vcells, (double) mp->left, (double) mp->top,
            (double) mp->cellWidth, (double) mp->cellHeight);
        return RValue_makeBool(false);
    }
    bool startBlocked = mp->cells[cxs * mp->vcells + cys] != 0;
    bool goalBlocked = mp->cells[cxg * mp->vcells + cyg] != 0;
    if (startBlocked || goalBlocked) {
        fprintf(stderr,
            "MP grid path failed: blocked start=(%.1f,%.1f)->(%d,%d):%d goal=(%.1f,%.1f)->(%d,%d):%d\n",
            (double) xstart, (double) ystart, cxs, cys, startBlocked,
            (double) xgoal, (double) ygoal, cxg, cyg, goalBlocked);
        return RValue_makeBool(false);
    }
    ...
    if (!result) {
        fprintf(stderr,
            "MP grid path failed: no route start=(%.1f,%.1f)->(%d,%d) goal=(%.1f,%.1f)->(%d,%d) diagonals=%d\n",
            (double) xstart, (double) ystart, cxs, cys,
            (double) xgoal, (double) ygoal, cxg, cyg, allowdiag);
        free(dist); free(qq);
        return RValue_makeBool(false);
    }
...
void VMBuiltins_registerAll(VMContext* ctx) {
    ...
    // Vertex formats/buffers used by WAD17's fixed-perspective sprite helpers.
