...
// Mounts the browser's OPFS at "/spider_donut" in the WASMFS virtual filesystem.
int mountOpfs(void) {
    ...
    int rc = wasmfs_create_directory("/spider_donut", 0777, opfs);
    if (rc != 0) {
        fprintf(stderr, "Failed to mount OPFS at /spider_donut: %s\n", strerror(errno));
        return -1;
    }
    return 0;
}
...
// gamePath: WASMFS path to the data.win to load (example: "/spider_donut/games/undertale/data.win").
// savesPath: WASMFS directory where saves should live (example: "/spider_donut/saves/undertale" - Created if it does not exist).
void startRunner(const char* gamePath, const char* savesPath) {
    ...
    // Bundle path = directory containing data.win, e.g. "/spider_donut/games/undertale/".
    // Save path = whatever the worker passed in, e.g. "/spider_donut/saves/undertale/".
