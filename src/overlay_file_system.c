...
// Resolves a read-only bundle path. The data.win directory wins; an optional
// shared bundle directory is consulted only when the file is absent locally.
static char* resolveBundlePath(OverlayFileSystem* ofs, const char* normalizedPath) {
    char* bundleFull = joinPath(ofs->bundlePath, normalizedPath);
    if (pathExists(bundleFull) || ofs->fallbackBundlePath == nullptr) return bundleFull;

    free(bundleFull);
    return joinPath(ofs->fallbackBundlePath, normalizedPath);
}
...
static char* resolveForRead(OverlayFileSystem* ofs, const char* relativePath) {
    ...
    if (ofs->fallbackBundlePath != nullptr && strncmp(normalized, ofs->fallbackBundlePath, strlen(ofs->fallbackBundlePath)) == 0) return normalized;
    ...
    free(saveFull);
    char* bundleFull = resolveBundlePath(ofs, normalized);
    free(normalized);
    return bundleFull;
}
...
static char* overlayResolvePath(FileSystem* fs, const char* relativePath) {
    ...
    char* full = resolveBundlePath(ofs, normalized);
    free(normalized);
    return full;
}
...
static FileSystemDirEntry* overlayListDirectory(FileSystem* fs, const char* relativeDirPath) {
    ...
    if (isAbsolute(normalized)) {
        ...
        free(bundleFull);
        if (ofs->fallbackBundlePath != nullptr) {
            char* fallbackFull = joinPath(ofs->fallbackBundlePath, normalized);
            listSingleDir(&list, fallbackFull);
            free(fallbackFull);
        }
    }
    free(normalized);
...
void OverlayFileSystem_setFallbackBundlePath(OverlayFileSystem* fs, const char* fallbackBundlePath) {
    if (fs == nullptr) return;
    free(fs->fallbackBundlePath);
    fs->fallbackBundlePath = fallbackBundlePath != nullptr ? withTrailingSlash(fallbackBundlePath) : nullptr;
}
