...
typedef struct {
    ...
    // If true, AUDO entries retain file offsets and are read only while a sound
    // is being decoded. This avoids retaining large embedded-audio chunks.
    bool lazyLoadAudio;
    // If true, parsed chunks are read through the buffered FILE instead of
    // being copied wholesale into temporary buffers. Intended for targets with
    // tight address-space limits such as PS3.
    bool streamChunks;
...
// Returns an AUDO entry's bytes. When outOwned is true, the caller must free
// the returned pointer. nullptr indicates an invalid entry or I/O failure.
const uint8_t* DataWin_acquireAudoData(DataWin* dataWin, uint32_t entryIndex, bool* outOwned);
