...
bool WAV_ParseFileData(uint8_t const* data, size_t dataSize, WAVFile* outFile) {
  if (data == nullptr || outFile == nullptr || dataSize < 44) return false;
  WAVFile file = {0};
  ...
  if (memcmp(file.header.file_id, "RIFF", 4) != 0 ||
      memcmp(file.header.format, "WAVE", 4) != 0 ||
      memcmp(file.header.data_id, "data", 4) != 0 ||
      file.header.data_size > dataSize - 44) {
    return false;
  }
