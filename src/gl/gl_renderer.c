...
static void glInit(Renderer* renderer, DataWin* dataWin) {
    ...
    // Desktop RGBA pages are four times larger than the PS3's paletted pages,
    // and Chapter 4 can touch more than twelve in one scene. Keep desktop
    // caching unbounded; the PS3 legacy renderer has the real console budget.
...
static void glEvictTexturePage(GLRenderer* gl, uint32_t pageId) {
    if (!gl->textureLoaded[pageId] || pageId >= gl->originalTexturePageCount) return;
...
static void glMakeTextureCacheRoom(GLRenderer* gl, uint32_t incomingPageId, size_t incomingBytes) {
    ...
    while (gl->textureBytesResident + incomingBytes > gl->textureCacheBudget) {
        ...
        for (uint32_t i = 0; i < gl->originalTexturePageCount; i++) {
            ...
            if (gl->batchCount > 0 && gl->glTextures[i] == gl->currentTextureId) continue;
        ...
        glEvictTexturePage(gl, candidate);
    }
}
...
bool GLRenderer_ensureTextureLoaded(GLRenderer* gl, uint32_t pageId) {
    ...
    glMakeTextureCacheRoom(gl, pageId, pageBytes);
...
static bool glSurfaceUpdatePixels(Renderer* renderer, int32_t surfaceId, const uint8_t* rgba, int32_t width, int32_t height) {
    GLRenderer* gl = (GLRenderer*) renderer;
...
static void glDrawTexturedVertices(Renderer* renderer, uint32_t texHandle,
                                   const RendererVertex2D* vertices,
                                   int32_t vertexCount, int32_t primitiveKind) {
    // GameMaker's pr_trianglestrip constant is 5. Chapter 4 submits exactly
    // four vertices in TL,TR,BL,BR order for its fixed-perspective quad.
    if (vertices == nullptr || vertexCount != 4 || primitiveKind != 5) return;
    GLRenderer* gl = (GLRenderer*) renderer;
    TexturePageItem* tpag = nullptr;
    GLuint texId = 0;
    int32_t texW = 0, texH = 0;
    if (!glResolveTextureHandle(gl, texHandle, &tpag, &texId, &texW, &texH)) return;

    const RendererVertex2D* tl = &vertices[0];
    const RendererVertex2D* tr = &vertices[1];
    const RendererVertex2D* bl = &vertices[2];
    const RendererVertex2D* br = &vertices[3];
    emitTexturedQuad(gl, texId,
        tl->x, tl->y, tr->x, tr->y, br->x, br->y, bl->x, bl->y,
        tl->u, tl->v, br->u, br->v,
        (uint8_t) BGR_R(tl->color), (uint8_t) BGR_G(tl->color), (uint8_t) BGR_B(tl->color),
        (uint8_t) BGR_R(tr->color), (uint8_t) BGR_G(tr->color), (uint8_t) BGR_B(tr->color),
        (uint8_t) BGR_R(br->color), (uint8_t) BGR_G(br->color), (uint8_t) BGR_B(br->color),
        (uint8_t) BGR_R(bl->color), (uint8_t) BGR_G(bl->color), (uint8_t) BGR_B(bl->color),
        tl->alpha);
}
...
Renderer* GLRenderer_create(void) {
    ...
    glVtable.surfaceUpdatePixels = glSurfaceUpdatePixels;
    ...
    glVtable.drawTexturedVertices = glDrawTexturedVertices,
