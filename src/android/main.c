...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, MAYBE_UNUSED void* reserved) {
    ...
    jclass localCls = (*env)->FindClass(env, "net/perfectdreams/spiderdonut/android/SpiderDonutNative");
    if (localCls == nullptr) {
        LOGE("JNI_OnLoad: FindClass failed for SpiderDonutNative");
        return JNI_ERR;
    }
...
#define JNI_FN(name) Java_net_perfectdreams_spiderdonut_android_SpiderDonutNative_##name
...
JNIEXPORT jboolean JNICALL JNI_FN(startRunner)(JNIEnv* env, MAYBE_UNUSED jclass cls, jstring jDataWinPath, jstring jSavesPath, jint jOsType, jint jHostFramebuffer) {
    ...
    arrput(gameArgs, safeStrdup("spider_donut")); // Synthetic argv[0]
...
#define SPIDER_DONUT_DROID_CONTINUE 0
#define SPIDER_DONUT_DROID_SHOULD_EXIT 1
#define SPIDER_DONUT_DROID_CONTINUE_NO_SWAP 2
...
JNIEXPORT jint JNICALL JNI_FN(stepAndDraw)(MAYBE_UNUSED JNIEnv* env, MAYBE_UNUSED jclass cls, jint winW, jint winH, jfloat deltaTime) {
    ...
    if (runner == nullptr)
        return SPIDER_DONUT_DROID_SHOULD_EXIT;
    ...
    if (runner->pendingWorkingDirectory != nullptr && runner->pendingLaunchParameters != nullptr) {
        ...
        return ok ? SPIDER_DONUT_DROID_CONTINUE : SPIDER_DONUT_DROID_SHOULD_EXIT;
    }
    ...
    return shouldSwap ? SPIDER_DONUT_DROID_CONTINUE : SPIDER_DONUT_DROID_CONTINUE_NO_SWAP;
}
